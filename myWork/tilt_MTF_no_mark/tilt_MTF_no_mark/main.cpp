...
void report(string path, const string& data){
	ofstream data_file;
	data_file.open("D:/NTS3Image/auto_roi_failed_images/pic/tilt_MTF_no_mark/param_data.csv", ofstream::app | ofstream::out);
	if (data_file.is_open()){
		path += "\n";
		data_file << path;
		
		string table_head = "angle,MTF_v\n";
...
void drawOsd(Mat& display, const Rect& rect, double mtf)
{
	int fontFace = CV_FONT_HERSHEY_SIMPLEX;
	//根据0402, 20M的像素大小, 按比例设置字体大小
	double fontScale = (double)display.cols * 2 / 5120 + 0.5;
	int thickness = 5 * display.cols / 5120 + 0.5;

	string text = (boost::format("%.2f") % mtf).str();
	Rect r_value = drawString(display, text, Point(rect.x + (rect.width >> 1), rect.y + (rect.height >> 1)), CV_RGB(0, 0, 255), fontScale, thickness, fontFace);

	rectangle(display, rect, CV_RGB(0, 0, 255), 2);

	static int save_cnt = 0;
	string path = (boost::format("D:/NTS3Image/auto_roi_failed_images/pic/tilt_MTF_no_mark/show/MTF%d.bmp") % save_cnt++).str();
...
/************************************************************************************/
int main(void){
	string p = "D:/NTS3Image/auto_roi_failed_images/pic/tilt_MTF_no_mark/color/";
	vector<string> img_names = { "P0425151916.bmp", "P0425151918.bmp" };
	...
	for (auto i = 0; i < img_names.size(); i++){
		...
		Rect roi_pos = Rect(image_width >> 2, image_height >> 2, image_width >> 1, image_width >> 1);

		Mat rotate_img = src.clone();

		double step_angle = 1, start_angle = step_angle, total_angle = 360;//360;

		string data = "";

		for (double angle = 0.0; angle < total_angle; angle += step_angle){
			double cur_angle = angle + start_angle;
			Mat ROI = getMatRect(rotate_img, roi_pos);
			
			double mtf = -1.0;
			MTFHistogram(mtf, 3, ROI, 0.15);
			
			data += (boost::format("%.1f,%g\n") % cur_angle % mtf).str();
			drawOsd(rotate_img, roi_pos, mtf);

			rotate_image(src, rotate_img, cur_angle);
		}
